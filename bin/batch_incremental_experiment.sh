...
do
	...
	/home/amber/stew/slam++/bin/slam_incre_clustering_lib -i sorted.g2o >&1 | tee /home/amber/stew/slam++/bin/clustering_output_"$SEED".txt
...
cd ../slam++/bin/
cp clustering_output_*.txt clustering_analysis_*.txt output_*.txt full_analysis_*.txt combined_score_plot_*.png result_* ../../test_backend/slampp_"$FILENAME"_"$OUTLIER_STRATETY"
