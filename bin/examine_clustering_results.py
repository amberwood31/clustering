...
#outlier_quantity = 20
    ...
    if len(sys.argv) > 1:
        ...
        inlier_quantity = int(sys.argv[4])
        outlier_quantity = int(sys.argv[5])   
    ...
    with open(output_path, encoding="utf-8") as obj:
        while edge_counter < lc_edge_quantity: # should always be true, since some edges will be deleted during the incremental clustering
            ...
            if line != '\n':
                ...
                line_splitted = line_stripped.split(' ')
                if line_splitted != ['']:
                    clusters[-1].append([int(line_splitted[0]), int(line_splitted[1])])
    ...
    content =open(rejected_loops_path, 'r')
    ...
    for line in lines:
        ...
        line_splitted = line_stripped.split(' ')
        #print(line_splitted)
        rejected_loops.append([line_splitted[0], line_splitted[1]])
                #print('edge counter: ', edge_counter)
